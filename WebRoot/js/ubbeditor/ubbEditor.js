function ubbEditor(_id)
{
  ...
  this.tToolbarSets['myeditor'] = ['Bold','Italic','Underline','Smiley','separator','JustifyLeft','JustifyCenter','JustifyRight','JustifyFull','separator','Picload','Link','Unlink','separator','Source','separator','Add','Minus'];	//user-defined toolbar
  this.tSystem = 'UBBEditor';
  this.tVersion = '1.0.1.0';
  this.tAuthor = 'Jetiben';
  this.tWebSite = 'http://www.ubbeditor.com/';
  this.initHeight = 0;	//初始化高度
  this.gap = 100;	//扩展和缩回高度
  this.action = '';	//处理文件上传Action对应的URL
    ...
    if (tstrers)
    {
      ...
      var tReplaceAry = [
        ...
        [/<img\s[^>]*?>/igm, function($0) {
          ...
          //目前[img]不支持height,width属性
          var attr = '=';
          var height;
          (height = tObj1.getAttribute('height')) || (height = tObj1.style.height);
          attr += (height ? height.replace('px','') : '');
          var width;
          (width = tObj1.getAttribute('width')) || (width = tObj1.style.width);
          attr += ','+(width ? width.replace('px','') : '')
          if (tObj1.getAttribute('src')) tString = '[img'+attr+']' + tObj1.getAttribute('src') + '[/img]';
...
//        [/\[img\]([^\[]*?)\[\/img\]/igm, '<img src="$1" />', true]
        [/\[img.*?\]([^\[]*?)\[\/img\]/igm, function($0,$1){
//        	alert($0+" "+$1);
        	var tString = '';
        	var img = $0;
        	var src = $1;
        	var index1 = img.indexOf('=');
        	var index2 = img.indexOf(']', index1);
        	var attr = img.substring(index1+1, index2).split(',');
//        	alert(attr);
        	if(attr) 
        		tString = '<img height="'+attr[0]+'" width="'+attr[1]+'" src="'+src+'">';
        	else 
        		tString = '<img src="'+src+'" />';
        	return tString;
        }, true]		
  ...
  this.tShowSource = function()
  {
    if (this.tEditState == 1)
    {
      ...
      tSourceImageObj.className = 'editor_botton';
  ...
  this.tShowNormal = function()
  {
    if (this.tEditState == 0)
    {
      ...
      tSourceImageObj = this.tEI(this.tid + '-ubbEditorToolbar-Source');
//      tSourceImageObj.onmouseover = function() {this.className = 'editor_botton'};
//      tSourceImageObj.onmouseout = function() {this.className = ''};
  ...
  this.tLoadToolbar = function(_obj)
  {
    ...
    if (tObj1)
    {
      ...
      for (var tKey1 in tArray1)
      {
        ...
        else if(tnKey == 'Source'){
        	tHTMLString1 += '<input id="' + this.tid + '-ubbEditorToolbar-' + tnKey + '" type="button" onclick="' + this.tName + '.tsetCommand(\'' + tnKey + '\');" value="源码" class="editor_botton"/>';
        }else if(tnKey == 'Minus'){
        	tHTMLString1 += '<input id="' + this.tid + '-ubbEditorToolbar-' + tnKey + '" type="button" onclick="' + this.tName + '.tsetCommand(\'' + tnKey + '\');" value="回缩" class="editor_botton"/>';
        }else if(tnKey == 'Add'){
        	tHTMLString1 += '<input id="' + this.tid + '-ubbEditorToolbar-' + tnKey + '" type="button" onclick="' + this.tName + '.tsetCommand(\'' + tnKey + '\');" value="扩展" class="editor_botton"/>';
        }
  ...
  this.tLoadPicTable = function()
  {
	var nEditor = this;
	
	if(nEditor.action == undefined || nEditor.action == ''){
  		alert("上传处理URL为空，无法处理上传图片！");
  		return false;
  	}
	  
	var tPicTableHTML = '<table cellpadding="0" cellspacing="5" class="ubbEditorImageTable" width="150px">';
  	tPicTableHTML += '	<tr>';
  	tPicTableHTML += '    <td height="20" valign="top">' + ubbEditorLang.tPicLoad + '</td>';
    tPicTableHTML += '  </tr>';
  	tPicTableHTML += '	<tr>';
  	tPicTableHTML += ' 		<td><p id="file_info" class="hide"></p><input id="uploadBtn" type="button" value="&nbsp;上传&nbsp;"></td>';
  	tPicTableHTML += '	</tr>';
  	tPicTableHTML += '</table>';
  	this.tLoadMaskShow(tPicTableHTML);
...
//        multiple: true,//批量上传
        ...
        onSubmit: function(file, ext){
			...
			$('#file_info').text("文件："+file).removeClass("hide");
			$uploadBtn.val("上传中");
			this.disable();
			interval = window.setInterval(function() {
			    var text = $uploadBtn.val();
			    if (text.length < 10) {
			        $uploadBtn.val(text + '.');
			    } else {
			        $uploadBtn.val('上传中');
...
//            alert(response.pic);
            $('#file_info').text("").addClass("hide");
            window.clearInterval(interval);
            $uploadBtn.val('上传');
			this.enable();
			nEditor.tLoadMaskClose();
			var upload_dir = "http://upload.psf100.com/pic/";
			var pic = upload_dir + response.pic.picid + "_3.jpg";
			var imgHTML = "<img width='300px' height='300px' src='" +pic+ "' alt='" +response.pic.filename+ "'/>";
			nEditor.tinsertHTML(imgHTML);
        }
    });
  };
  this.addHeight = function()
  {
	var div = document.getElementById(this.tid + '-div');
	var div_frame = document.getElementById(this.tid + '-iframe');
	var div_area = document.getElementById(this.tid + '-textarea');
	div.style.height = (parseInt(div.style.height.replace('px','')) + this.gap)+'px';
	div_frame.style.height = (parseInt(div_frame.style.height.replace('px','')) + this.gap) + 'px';
	div_area.style.height = (parseInt(div_area.style.height.replace('px','')) + this.gap) + 'px';
  };
  this.minusHeight = function()
  {	
	var div = document.getElementById(this.tid + '-div');
	var div_frame = document.getElementById(this.tid + '-iframe');
	var div_height = parseInt(div.style.height);
	var div_frame_height = parseInt(div_frame.style.height);
	var div_area = document.getElementById(this.tid + '-textarea');
	if(div_frame_height > (this.initHeight-this.tPanelHeight-5)){
		div_frame.style.height = (div_frame_height - this.gap) + 'px';
		div.style.height = (div_height - this.gap) + 'px';
		div_area.style.height = (parseInt(div_area.style.height.replace('px','')) - this.gap) + 'px';
	}
  };
   ...
   * 初始化编辑器
   * @param {Object} _name
   * @param {Object} _baseURL	插件目录
   * @param {Object} _action	上传文件处理URL
   * @memberOf {TypeName} 
   */
  this.tInit = function(_name, _baseURL, _action)
  {
    this.tName = _name;
    ...
    if (tObj1)
    {
      ...
      this.initHeight = tObj1Height;	//设置初始化高度
      ...
      tTextarea1.setAttribute('wrap', 'physical');	//Change and new row for English
...
//      tTextarea1.style.overflowY = 'hidden';
